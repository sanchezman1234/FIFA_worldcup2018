...
world_cup %<>% mutate(total = point + point2)
...
w1 <- world_cup %>% select(Name,`Round 2 - Top 8`) %>% separate(`Round 2 - Top 8`,z,sep = ",")

data_sweet16 <- world_cup %>% select(Name,total,round2 = point2) %>% right_join(w1, by = "Name")
